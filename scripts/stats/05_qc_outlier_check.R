# scripts/stats/05_qc_outlier_check.R
...
out_flags  <- here::here("results","qc","outlier_flags.csv")
out_pca    <- here::here("results","qc","pca_outliers.png")
out_scree  <- here::here("results","qc","pca_scree.png")

dir.create(here::here("results","qc"), recursive = TRUE, showWarnings = FALSE)

# -------------------- PARAMETERS (easy to change) --------------------
# Rule 1: simple cut on PC scores (like on your plot)
PC1_LIMIT <- 60     # flag if |PC1| > 60
PC2_LIMIT <- 40     # flag if |PC2| > 40

# Rule 2: robust distance in PC space (Mahalanobis on first K PCs)
...
# IMPORTANT: matrix is already normalized & scaled (Pareto), so no extra centering/scaling
pc <- prcomp(t(X2), center = FALSE, scale. = FALSE)

scores <- as.data.frame(pc$x)
expl   <- (pc$sdev^2) / sum(pc$sdev^2)

# -------------------- OUTLIER RULE 1: hard cut on PC1/PC2 --------------------
flag_rule1 <- (abs(scores$PC1) > PC1_LIMIT) | (abs(scores$PC2) > PC2_LIMIT)

# -------------------- OUTLIER RULE 2: robust distance in PC space --------------------
# Use first K PCs (or as many as available)
K <- min(N_PCS_FOR_DISTANCE, ncol(scores))
S <- as.matrix(scores[, seq_len(K), drop = FALSE])

# Mahalanobis distance (classical; на нормализованных данных работает стабильно)
# If singular, add tiny jitter to diagonal
covS <- cov(S)
...
# cutoff по хи-квадрат с K степенями свободы
...
# attach simple group if present
