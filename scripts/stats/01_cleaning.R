...
#
# Purpose:
#   Basic cleaning of NEG LC–MS feature table before any
#   normalization or statistics.
#
#   Steps:
#     1) Load LC–MS feature table (NEG mode)
...
#     5) Impute remaining NAs with half of row minimum (half-min)
#     6) Save cleaned table and summary statistics
#     7) Save session info for reproducibility
#
# Rationale:
#   • Technical samples are not biological → exclude from statistics.
#   • Too many NAs make features unreliable → filter by threshold.
#   • In LC–MS, NA ≈ “below detection limit” → half-min keeps scale.
#   • QC plots + logs = transparency and reproducibility.
#
# Input :
#   data/processed/feature_table_neg.csv
# Output:
#   data/processed/feature_table_neg_clean.csv
#   results/qc/cleaning_neg/*.png, *.csv
# -------------------------------------------------------------
...
qc_dir      <- here("results", "qc",   "cleaning_neg")
...
# They are not biological samples and will bias results.
...
if (length(technical)) {
  ...
  ft <- ft |>
    dplyr::select(-dplyr::all_of(technical))
}
...
# -------------------- Save cleaned table ----------------------
readr::write_csv(
  ft |> tibble::rownames_to_column(var = "feature_id"),
  output_path
)
...
summary_tbl <- tibble::tibble(
  ...
  before = c(nrow(feature_table_raw),
             ncol(feature_table_raw),
             sum(is.na(feature_table_raw))),
  after  = c(nrow(ft),
             ncol(ft),
             sum(is.na(ft)))
)
