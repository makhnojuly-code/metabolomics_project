...
# Normalize cleaned LC–MS (NEG) feature table for statistics.
# Pipeline:
#   1) PQN  – fix total concentration differences between samples
#   2) log2 – compress large values and stabilize variance
#   3) Pareto – balance feature influence for PCA / PLS-DA
#
# Input  : data/processed/feature_table_neg_clean.csv  (features x samples)
# Output : data/processed/feature_table_neg_processed_for_stats.csv
#
#  Missing values were already imputed in CLEANING (half-min).
...
if (any(grepl("(?i)QC|MM8|ACN", colnames(X)))) {
  warning("Technical samples (QC/MM8/ACN) detected. Remove them before normalization.")
}
...
# Save PQN info (QC)
write.csv(data.frame(sample = colnames(Xpqn), pqn_factor = as.numeric(pqn_factor)),
          file.path(QC_DIR, "pqn_factors.csv"), row.names = FALSE)
write.csv(data.frame(feature = rownames(Xpqn), ref_median = as.numeric(ref)),
          file.path(QC_DIR, "pqn_reference_row_medians.csv"), row.names = FALSE)

# -------------------- Step 2: log2 transform --------------------
#  reduce dominance of very large peaks and stabilize variance

if (EPS_MODE == "adaptive") {
  min_pos <- suppressWarnings(min(Xpqn[Xpqn > 0 & is.finite(Xpqn)], na.rm = TRUE))
...
# Save eps (QC)
write.csv(data.frame(metric = "eps_used", value = eps),
          file.path(QC_DIR, "eps_used.csv"), row.names = FALSE)
...
pareto <- sweep(sweep(logX, 1, row_center, "-"), 1, sqrt(row_scale), "/")
...
norm_summary <- data.frame(
  metric = c("n_features_in", "n_samples_in", "pareto_mode", "eps_mode", "eps_value"),
  value  = c(nrow(X), ncol(X), PARETO_MODE, EPS_MODE, eps)
)
write.csv(norm_summary, file.path(QC_DIR, "normalization_summary.csv"), row.names = FALSE)

# Save run parameters (useful for reproducibility)
write.csv(
  data.frame(param = c("EPS_MODE","EPS_FIXED","PARETO_MODE"),
