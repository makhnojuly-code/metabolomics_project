...
Script 02A performs automated Mummichog analysis using the MetaboAnalystR package.

It replaces the manual upload to the MetaboAnalyst web interface.
The script loads the file metabo_mummichog_NEG.txt, applies the same processing parameters as the online tool 
(ppm tolerance, enrichment mode, p-value cutoff), and runs Mummichog locally.

It automatically generates the same two output files as the web version:
