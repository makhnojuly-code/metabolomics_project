...
cols_bpc  <- palette_soft(length(files_bpc)); names(cols_bpc) <- files_bpc
...
invisible(lapply(files_bpc, function(f) {
  ...
  lines(d$rt, d$intensity, col = cols_bpc[f], lwd = 1.2)
}))
...
invisible(lapply(files_bpc, function(f) {
  d <- bpc_ds[bpc_ds$file == f]
  lines(d$rt, d$intensity, col = cols_bpc[f], lwd = 1.6)   # толще линия
}))
legend("topright", legend = files_bpc, col = cols_bpc, lwd = 1, cex = 0.7, bty = "n")
dev.off()

# ---- 3a) OPTIONAL: BPC in blocks of 12 files for readability ----------------
# Создаст qc_bpc_block_XX.png; отключи, если не нужно
files_bpc <- unique(bpc_ds$file)
...
# Colors per file (оставлю пастельные, можно переключить на яркие при желании)
files_tic <- sort(unique(tic_df$file))
cols_tic  <- palette_soft(length(files_tic), alpha = 0.8)  # <- поменяй на palette_bright(...) если хочешь
names(cols_tic) <- files_tic
bx_cols <- cols_tic[files_tic]
